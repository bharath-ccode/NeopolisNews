 ...
 *  whatever subtypes/localities the admin selected on /admin/business-
 *  discovery. POST { subtypeKeys: string[], localities: string[] } (both
 *  required, non-empty — the UI won't submit without both, and we don't
 *  trust that alone). Protected by middleware (/api/admin/*). */
export async function POST(req: NextRequest) {
  ...
  if (localities.length === 0) {
    return NextResponse.json({ error: "Select at least one locality to search." }, { status: 400 });
  }

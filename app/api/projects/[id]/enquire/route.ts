...
export async function POST(
  ...
  if (!toEmail)
    return NextResponse.json({ error: "Unable to deliver enquiry." }, { status: 500 });
  ...
  await Promise.all([
    ...
    resend.emails.send({
      ...
      html:    buildEmail({ projectName: project.project_name, senderName, senderPhone, message }),
    }),
  ]);
...
<div style="max-width:520px;margin:40px auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.06)">
  <div style="background:#0f2a4a;padding:24px 32px">
    <p style="color:#7eb3e8;font-size:11px;margin:0 0 4px;text-transform:uppercase;letter-spacing:.05em">NeopolisNews — Project Enquiry</p>
    <h1 style="color:white;margin:0;font-size:18px">${p.projectName}</h1>
  </div>

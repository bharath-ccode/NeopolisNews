...
const PROPERTY_TYPES: Record<SubCategory, { value: PropertyType; label: string }[]> = {
  ...
  retail:  [{ value: "shop",         label: "Shop / Retail"  }],
  office:  [{ value: "office_space", label: "Office Space"   }],
};
...
export default function BrokerEditListing() {
  ...
  useEffect(() => {
    fetch(`/api/broker/listings/${params.id}`)
      .then((r) => r.json())
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .then((d: any) => {
        if (!d || d.error) { router.replace("/broker/listings"); return; }
        setSubCategory(d.sub_category ?? "residential");
        setListingType(d.listing_type ?? "sale");
        setPropertyType(d.property_type ?? "apartment");
        setTower(d.tower ?? "");
        setFloorNumber(d.floor_number?.toString() ?? "");
        setUnitNumber(d.unit_number ?? "");
        setBedrooms(d.bedrooms ?? "2");
        setBathrooms(d.bathrooms ?? "2");
        setCarpetArea(d.carpet_area_sqft?.toString() ?? "");
        setParking(d.parking ?? "1");
        setFurnished(d.furnished ?? "semi-furnished");
        setPrice(d.price ?? "");
        setDeposit(d.deposit ?? "");
        setAvailableFrom(d.available_from ?? "");
        setDescription(d.description ?? "");
        setAmenities(d.amenities ?? []);
        setOwnerName(d.owner_name ?? "");
        setContactPhone(d.contact_phone ?? "");
        setContactPref(d.contact_preference ?? "both");

        if (d.is_standalone) {
          setProjectMode("standalone");
          setStandaloneDesc(d.standalone_description ?? "");
        } else if (d.project_id) {
          setProjectMode("existing");
          // Mark the project as selected by id
          setSelectedProject({ id: d.project_id, projectName: d.project_name ?? "" } as Project);
        } else if (d.project_name) {
          setProjectMode("new");
          setCustomProjectName(d.project_name);
        }
      })
      .catch(() => router.replace("/broker/listings"))
  ...
  async function submit() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/broker/listings/${params.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ownerName, contactPhone, contactPreference: contactPref,
          subCategory, listingType,
          projectId:   projectMode === "existing" ? selectedProject?.id : null,
          projectName: projectMode === "existing" ? selectedProject?.projectName
                       : projectMode === "new" ? customProjectName : null,
          isStandalone: projectMode === "standalone",
          standaloneDescription: projectMode === "standalone" ? standaloneDesc : null,
          tower: tower || null, floorNumber: floorNumber || null, unitNumber: unitNumber || null,
          propertyType,
          bedrooms: isResidential ? bedrooms : null,
          bathrooms: isResidential ? bathrooms : null,
          carpetAreaSqft: carpetArea || null,
          parking, furnished: propertyType !== "plot" ? furnished : null,
          availableFrom: availableFrom || null, amenities, price,
  ...
  if (done) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <h2 className="text-xl font-extrabold text-gray-900 mb-2">Listing Updated</h2>
        <p className="text-gray-500 text-sm mb-6">Changes are live on NeopolisNews.</p>
        <button onClick={() => router.push("/broker/listings")} className="btn-primary text-sm">
          Back to Listings <ChevronRight className="w-4 h-4" />
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-0.5">Updates go live immediately.</p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3 pt-2">
              <div>
                ...
                <input type="text" value={tower} onChange={(e) => setTower(e.target.value)} placeholder="Optional" className={inputClass} />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1.5">Floor</label>
                <input type="number" value={floorNumber} onChange={(e) => setFloorNumber(e.target.value)} placeholder="Optional" className={inputClass} />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1.5">Unit No.</label>
                <input type="text" value={unitNumber} onChange={(e) => setUnitNumber(e.target.value)} placeholder="Optional" className={inputClass} />
              </div>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="e.g. 85,00,000" className={inputClass} />
            </div>
            {listingType === "rent" && (
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5">Security Deposit</label>
                <input type="text" value={deposit} onChange={(e) => setDeposit(e.target.value)} placeholder="Optional" className={inputClass} />
              </div>
            )}
            <div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)}
                placeholder="Describe the property…" rows={4} className={inputClass} />
            </div>
            <div>
        ...
        {step === 5 && (
          <div className="space-y-4">
            <p className="text-xs text-gray-500 bg-cyan-50 border border-cyan-100 rounded-lg px-3 py-2">
              This contact is shown to buyers and tenants on the listing.
            </p>
            <div>
            ...
            <div>
              ...
              <input type="tel" value={contactPhone} onChange={(e) => setContactPhone(e.target.value)} placeholder="10-digit number" className={inputClass} />
            </div>
            <div>

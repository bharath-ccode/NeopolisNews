...
/** One movie's showtimes as individual Book buttons — falls back to the
 *  movie's own bms_url (no time) when no showtimes have been entered yet,
 *  and each showtime falls back to the movie's bms_url when it has no
 *  dedicated link of its own. */
function MovieRow({
  movie, cinema, isoDate,
}: {
  movie: Movie;
  cinema: Cinema;
  isoDate: string;
    ...
    <div className="border border-gray-100 rounded-xl p-3">
      <div className="flex items-start gap-3">
        {movie.poster_url ? (
          ...
          <img src={movie.poster_url} alt={movie.title} className="w-10 h-14 rounded-lg object-cover shrink-0" />
        ) : (
        ...
        <div className="min-w-0 flex-1">
          <p className="font-bold text-sm text-gray-900 truncate">{movie.title}</p>
          {movie.languages.length > 0 && (
            <p className="text-xs text-gray-400 truncate">{movie.languages.join(" · ")}</p>
          )}

          {movie.show_times.length > 0 ? (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {movie.show_times.map((st) => {
                const url = st.bms_url || movie.bms_url;
                if (!url) return (
                  <span key={st.id} className="text-xs font-semibold text-gray-400 border border-gray-200 rounded-full px-2.5 py-1">
                ...
                return (
                  <a
                    ...
                    className="text-xs font-bold text-white bg-red-600 hover:bg-red-700 rounded-full px-2.5 py-1 transition-colors"
                  >
                    {formatTime12h(st.time)}
                  </a>
                );
            ...
            <a
              ...
              onClick={() => trackTicketClick({
                ...
                movieTitle: movie.title,
                showDate: isoDate,
                bmsUrl: movie.bms_url!,
                source: "cinemas_page",
              })}
              className="inline-flex items-center gap-1 text-xs font-bold text-red-600 hover:text-red-700 mt-1.5"
            >
...
export default function CinemasPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/cinemas?date=${isoDate}`)
      .then(r => r.json())
                ...
                {cinemas.map((cinema) => {
                  ...
                  return (
                    <div key={cinema.id} className="card p-6 space-y-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                      ...
                      {cinema.now_showing.length > 0 && (
                        <div className="space-y-2">
                          {cinema.now_showing.map((m) => (
                            <MovieRow key={m.id} movie={m} cinema={cinema} isoDate={isoDate} />
                          ))}
                        </div>
                      )}
                      ...
                      {bmsUrl ? (
                        ...
                        >
                          ...
                          {cinema.now_showing.length > 0 ? "View All Showtimes" : "View Showtimes & Book"}

...
- **Distance-from-you on business cards** — `businesses.latitude`/`longitude`, populated for free from the Places data already captured at discovery time (`business_discovery_candidates.lat`/`lng`, carried forward by `promoteCandidateToBusiness`); `/health` listing cards show straight-line distance (`lib/distance.ts`'s Haversine calc, no API call) from the browser's geolocation (`lib/useUserLocation.ts`) next to the phone number — ⚠️ requires `supabase/migrations/20260829_business_coordinates.sql` to backfill already-promoted businesses from their candidate row's coordinates

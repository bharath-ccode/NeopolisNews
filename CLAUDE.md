...
- **Daily poll** — admin-created question with 2–6 multiple-choice options (`/admin/polls`), each option optionally given a starting **seed vote count** at creation time (`poll_options.seed_votes`); one poll per `publish_date` like the cartoon; homepage panel + `/polls` archive; no sign-in required to vote — signed-in votes go through `poll_votes` (one per user per poll, changeable via upsert on `(poll_id, user_id)`), signed-out votes have no stable identity so they're tallied via an atomic RPC (`increment_poll_option_votes`) into `poll_options.anon_votes` instead, with the browser's own "already voted" state remembered in `localStorage` (`nn_poll_vote_${pollId}`) for UI continuity only; results shown as bars once the reader has voted; ⚠️ requires `supabase/migrations/20260826_polls.sql` and `supabase/migrations/20260830_polls_anonymous_voting.sql` to be run
